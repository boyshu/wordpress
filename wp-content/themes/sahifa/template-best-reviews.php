	...
	<div class="content">
		...
		<?php if ( ! have_posts() ) : ?>
			<div id="post-0" class="post not-found post-listing">
				...
				<div class="entry">
					<p><?php _e( '抱歉，您浏览的页面未找到。也许搜索能帮到您。', 'tie' ); ?></p>
					<?php get_search_form(); ?>
				</div>
			</div>
		<?php endif; ?>
		...
		<article <?php if( !empty( $rv['review'] ) ) echo $rv['review']; post_class('post-listing post'); ?>>
			...
			<div class="post-inner">
				...
				<div class="entry">
					<?php the_content(); ?>
					<?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( '页数:', 'tie' ), 'after' => '</div>' ) ); ?>
...
<img class="post-272125" src=<?php echo PostThumbURL(); ?> alt="<?php the_title(); ?>" />
<?php }else if (catch_that_image()) {?>
<img class="post-272125" src="<?php echo catch_that_image(); ?>" alt="<?php the_title(); ?>" />
<?php } ?>
							</a>
						</div><!-- post-thumbnail /-->
						...
						<div class="best-reviews-content">
							...
							<p class="post-meta">
								<span><?php _e( '作者: ' , 'tie' ); ?><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) )?>" title="<?php sprintf( esc_attr__( '查看所有文章 %s', 'tie' ), get_the_author() ) ?>"><?php echo get_the_author() ?> </a></span>
								<?php if( tie_get_option( 'post_date' ) && tie_get_option( 'time_format' ) != 'none' ): ?>		
									<?php __( '发表时间： ' , 'tie' ); ?><?php tie_get_time() ?>
								<?php endif; ?>	
								<span><?php _e( '分类： ' , 'tie' ); ?><?php printf('%1$s', get_the_category_list( ', ' ) ); ?></span>
							</p>
						</div>
					</li>
					<?php endwhile;
