...
class tie_author_widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p><em style="color:red;">这个小工具仅出现在单独文章 .</em></p>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'avatar' ); ?>">显示作者的头像 : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'social' ); ?>">现实社会化图标 : </label>
...
class author_post_widget extends WP_Widget {
	function author_post_widget() {
		...
		$this->WP_Widget( 'author_post_widget',theme_name .' - 帖子文章作者', $widget_ops );
	}
	function widget( $args, $instance ) {
		extract( $args );
		wp_reset_query();
	...
	function form( $instance ) {
		...
		<p><em style="color:red;">这个小工具仅出现在但文章页.</em></p>
		
		<p>
			<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>">显示文章数目: </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'see_all' ); ?>">显示 ( 查看全部 ) 链接 : </label>
