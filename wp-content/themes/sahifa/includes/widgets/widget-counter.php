...
class tie_counter_widget extends WP_Widget {
	...
	function tie_counter_widget() {
		...
		$this->WP_Widget( 'counter-widget',theme_name .' - 社交计数', $widget_ops, $control_ops );
	}
			...
			<?php if( $rss_id ): ?>
				<li class="rss-subscribers">
					<a href="<?php echo $rss_id ?>"<?php echo $new_window ?>>
						...
						<span><?php _e('订阅' , 'tie' ) ?><?php __('Subscribers' , 'tie' ) ?></span>
						<small><?php _e(' RSS 订阅' , 'tie' ) ?></small>
					</a>
				</li>
			<?php endif; ?>
			<?php if( $twitter_id ):
	...
	function form( $instance ) { ?>
		<p>
			<label for="<?php echo $this->get_field_id( 'new_window' ); ?>">在新窗口中打开链接：</label>
		...
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			<label for="<?php echo $this->get_field_id( 'rss' ); ?>"><strong>Feed</strong> 地址 :  </label>
		...
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			<label for="<?php echo $this->get_field_id( 'facebook' ); ?>"><strong>Facebook</strong> 页面地址 : </label>
		...
		if( empty($twitter_username) || empty($consumer_key) || empty($consumer_secret) || empty($access_token) || empty($access_token_secret)  )
				echo '<p style="display:block; padding: 5px; font-weight:bold; clear:both; background: rgb(255, 157, 157);">错误 : 前往 Twitter API OAuth 设置在 Tiepanel > Advanced tab .</p>';
		
		?>
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			<label for="<?php echo $this->get_field_id( 'twitter' ); ?>"><strong>Twitter</strong> 追随者计数: </label>
		...
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			...
			<small>链接必须如 http://www.youtube.com/user/username or http://www.youtube.com/channel/channel-name </small>
		...
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			...
			<small>链接必须如 http://vimeo.com/channels/username </small>
		...
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			...
			<small>链接必须如 http://dribbble.com/username</small>
		</p>
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			<label for="<?php echo $this->get_field_id( 'soundcloud' ); ?>"><strong>SoundCloud</strong> 用户配置文件地址 : </label>
			...
			<small>点击 <a href="http://themes.tielabs.com/docs/sahifa/index.htm#counter" target="_blank"><strong>社会化计数文档</strong></a> 了解更多 .</small>
		</p>
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			<label for="<?php echo $this->get_field_id( 'behance' ); ?>"><strong>Behance</strong> 用户配置文件地址 :  </label>
		...
		<p style="border-bottom: 1px solid #DDD;padding-bottom: 10px;">
			<label for="<?php echo $this->get_field_id( 'instagram' ); ?>"><strong>Instagram</strong> 用户配置文件地址 :  </label>
			...
			<small>Check <a href="http://themes.tielabs.com/docs/sahifa/index.htm#counter" target="_blank"><strong>社会化计数文档</strong></a> 了解更多 .</small>
		</p>
		<p style="background-color: #ecf7e7;margin: 10px 0;padding: 10px;color: #468847;border: 3px solid #d6e9c6;">需要更多的社交网络......看看我们的 <a href="http://codecanyon.net/item/arqam-retina-responsive-wp-social-counter-plugin/5085289?ref=TieLabs" target="_blank">Arqam插件</a> .</p>
	<?php
	}
}
