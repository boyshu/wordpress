...
class tie_flickr_photos extends WP_Widget {
	...
	function tie_flickr_photos() {
		...
		$this->WP_Widget( 'flickr_photos-widget',theme_name .' - Flickr相片分享', $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'no_of_photos' ); ?>">显示的照片数量 : </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'flickr_display' ); ?>">显示 ? </label>
			<select id="<?php echo $this->get_field_id( 'flickr_display' ); ?>" name="<?php echo $this->get_field_name( 'flickr_display' ); ?>" >
				<option value="latest" <?php if( !empty($instance['flickr_display']) && $instance['flickr_display'] == 'latest' ) echo "selected=\"selected\""; else echo ""; ?>>最新的</option>
				<option value="random" <?php if( !empty($instance['flickr_display']) && $instance['flickr_display'] == 'random' ) echo "selected=\"selected\""; else echo ""; ?>>随机的</option>
			</select>
		</p>
