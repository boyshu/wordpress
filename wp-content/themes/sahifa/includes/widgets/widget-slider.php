...
class tie_slider extends WP_Widget {
	...
	function tie_slider() {
		...
		$this->WP_Widget( 'tie-slider-widget',theme_name .' - 幻灯片', $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>">显示文章数: </label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'custom_slider' ); ?>">自定义幻灯片 : </label>
			...
			<?php else: ?>
			<span style="color:#FF0000;">先添加自定义滑块。</span>
			<?php endif; ?>
		</p>
	<?php
	}
