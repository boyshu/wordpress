<?php
/*
Plugin Name: 腾讯云COS同步插件（V5版本SDK）
Plugin URI: http://www.slmwp.com/cos-sync-plugins-v5
...
function _file_upload($object, $file, $opt = array()){
    ...
    if (@file_exists($file)) {
        ...
        $cosClient = new Qcloud\Cos\Client(
            [
                'region' => esc_attr($cos_options['regional']), #地域，如ap-guangzhou,ap-beijing-1
                'credentials' => [
...
function cos_add_setting_page(){
    add_options_page('腾讯云COS设置V5', '腾讯云COS设置V5', 'manage_options', __FILE__, 'cos_setting_page');
}
...
function cos_setting_page(){
    ...
    if (!empty($_POST) and $_POST['type'] == 'cos_set') {
        ...
        $options['ssl'] = (isset($_POST['ssl'])) ? 'true' : 'false';
    ...
    if(!empty($_POST) and $_POST['type'] == 'cos_sync_replace'){
        global $wpdb;
    ...
    $cos_ssl = esc_attr($cos_options['ssl']);
    $cos_ssl = ($cos_ssl == 'true');
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td><select name="regional">
                            <option value="ap-beijing-1" <?php if ($cos_regional == 'ap-beijing-1'){echo ' selected="selected"';}?>>北京一区（华北）</option>
                            <option value="ap-beijing" <?php if ($cos_regional == 'ap-beijing'){echo ' selected="selected"';}?>>北京</option>
                            <option value="ap-shanghai" <?php if ($cos_regional == 'ap-shanghai'){echo ' selected="selected"';}?>>上海（华东）</option>
                            <option value="ap-guangzhou" <?php if ($cos_regional == 'ap-guangzhou'){echo ' selected="selected"';}?>>广州（华南）</option>
                            <option value="ap-chengdu" <?php if ($cos_regional == 'ap-chengdu'){echo ' selected="selected"';}?>>成都（西南）</option>
                            <option value="ap-chongqing" <?php if ($cos_regional == 'ap-chongqing'){echo ' selected="selected"';}?>>重庆</option>
                            <option value="ap-singapore" <?php if ($cos_regional == 'ap-singapore'){echo ' selected="selected"';}?>>新加坡</option>
                            <option value="ap-hongkong" <?php if ($cos_regional == 'ap-hongkong'){echo ' selected="selected"';}?>>香港</option>
                            <option value="ap-toronto" <?php if ($cos_regional == 'ap-toronto'){echo ' selected="selected"';}?>>多伦多</option>
                            <option value="ap-frankfurt" <?php if ($cos_regional == 'ap-frankfurt'){echo ' selected="selected"';}?>>法兰克福</option>
                            ...
                            <option value="ap-siliconvalley" <?php if ($cos_regional == 'ap-siliconvalley'){echo ' selected="selected"';}?>>硅谷</option>
                            <option value="ap-ashburn" <?php if ($cos_regional == 'ap-ashburn'){echo ' selected="selected"';}?>>弗吉尼亚</option>
                            <option value="ap-bangkok" <?php if ($cos_regional == 'ap-bangkok'){echo ' selected="selected"';}?>>曼谷</option>
                            <option value="ap-moscow" <?php if ($cos_regional == 'ap-moscow'){echo ' selected="selected"';}?>>莫斯科</option>
                        </select>
                ...
                <tr>
                    <th>
                        <legend>是否启用安全证书链接</legend>
                    </th>
                    <td>
                        <input type="checkbox" name="ssl" <?php if ($cos_ssl) echo 'checked="TRUE"'; ?> />
                        <p>如果您的站点启用了安全证书，请启用该选项</p>
                    </td>
                </tr>
                <tr>
                    <th>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <p><b>注意：如果是首次同步，执行时间将会十分十分长（根据你的历史附件数量），有可能会因执行时间过长，页面显示超时或者报错。<br>
                                所以，建议那些几千上万附件的大神们，考虑官方的 <a href="https://www.qcloud.com/document/product/436/7133" target="_blank">同步工具</a></b></p>
                    </td>
                </tr>
            </table>
        </form>
        ...
        <form name="form1" method="post" action="<?php echo wp_nonce_url('./options-general.php?page=' . COS_BASEFOLDER . '/cos-sync.php'); ?>">
            <table class="form-table">
                ...
                <tr>
                    ...
                    <input type="hidden" name="type" value="cos_sync_replace">
                    <td>
                        <input type="submit" name="submit" value="开始替换"/>
                        <p><b>尽量使用纯域名，开头不要加<code>http://</code>，末尾不要加<code>/</code>，最重要的一点，记得备份！记得备份！！记得备份！！！</b></p>
                    </td>
                </tr>
            </table>
        </form>
